...
class Workspaces : public AModule, public EventHandler {
 public:
  ...
  // Monitor whose workspaces should be displayed by Waybar.
  // If "workspace-monitor" is configured, every Waybar instance
  // can display the same monitor's workspace list.
  auto getWorkspaceMonitor() const -> std::string {
    if (config_["workspace-monitor"].isString()) {
      return config_["workspace-monitor"].asString();
    }

    return m_bar.output->name;
  }
 ...
 private:
  ...
  static Json::Value createMonitorWorkspaceData(
  ...
  auto populateIconsMap(
      const Json::Value& formatIcons) -> void;

  static auto populateBoolConfig(
      const Json::Value& config,
      const std::string& key,
      bool& member) -> void;

  auto populateSortByConfig(
      const Json::Value& config) -> void;

  auto populateIgnoreWorkspacesConfig(
      const Json::Value& config) -> void;

  auto populateFormatWindowSeparatorConfig(
      const Json::Value& config) -> void;

  auto populateWindowRewriteConfig(
      const Json::Value& config) -> void;

  auto populateWorkspaceTaskbarConfig(
      const Json::Value& config) -> void;
  ...
  static std::optional<int> parseWorkspaceId(
  ...
  int windowRewritePriorityFunction(
  ...
  static std::pair<std::string, std::string> splitDoublePayload(
      std::string const& payload);

  static std::tuple<std::string, std::string, std::string>
  splitTriplePayload(std::string const& payload);
  ...
  void registerOrphanWindow(
      WindowCreationPayload create_window_payload);

  void initializeWorkspaces();
  void setCurrentMonitorId();

  void loadPersistentWorkspacesFromConfig(
      Json::Value const& clientsJson);

  void loadPersistentWorkspacesFromWorkspaceRules(
  ...
  std::map<std::string, SortMethod> m_sortMap = {
      {"ID", SortMethod::ID},
  ...
  std::vector<std::pair<Json::Value, Json::Value>>
  ...
  Gtk::Orientation m_taskbarOrientation =
      Gtk::ORIENTATION_HORIZONTAL;

  bool m_taskbarReverseDirection = false;

  util::EnumParser<ActiveWindowPosition>
      m_activeWindowEnumParser;

  ActiveWindowPosition m_activeWindowPosition =
      ActiveWindowPosition::NONE;

  std::map<std::string, ActiveWindowPosition>
