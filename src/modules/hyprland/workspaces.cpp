...
void Workspaces::initializeWorkspaces() {
  ...
  // Reset the currently displayed workspaces.
  ...
  const int activeWorkspaceId =
      m_ipc.getSocket1JsonReply("activeworkspace")["id"].asInt();

  // Find logical workspaces that are actually in use.
  //
  // 1 <-> 10
  // 2 <-> 11
  // ...
  // 9 <-> 18
  //
  // A pair is considered in use when either side has windows,
  // or when either side is currently active.
  std::set<int> logicalWorkspaces;

  for (const Json::Value& workspaceJson : workspacesJson) {
    const int workspaceId = workspaceJson["id"].asInt();

    if (workspaceId < 1 || workspaceId > 18) {
      continue;
    }

    const int logicalId =
        workspaceId <= 9 ? workspaceId : workspaceId - 9;

    const int windows = workspaceJson["windows"].asInt();

    if (windows > 0 || workspaceId == activeWorkspaceId) {
      logicalWorkspaces.insert(logicalId);
    }
  }

  // Create the real workspace for this monitor, or a visual paired
  // representation when the real workspace exists on the other monitor.
  for (int logicalId : logicalWorkspaces) {
    const int realId =
        m_bar.output->name == "HDMI-A-1"
            ? logicalId + 9
            : logicalId;

    bool alreadyQueued = std::ranges::any_of(
        m_workspacesToCreate,
        [realId](const auto& entry) {
          return entry.first["id"].asInt() == realId;
        });
    ...
    // Look for this workspace on either monitor.
    for (const Json::Value& workspaceJson : workspacesJson) {
      const int workspaceId = workspaceJson["id"].asInt();

      if (workspaceId != realId) {
        continue;
      }

      std::string workspaceName = workspaceJson["name"].asString();

      if (workspaceName.starts_with("special") && !showSpecial()) {
        break;
      }

      if (isWorkspaceIgnored(workspaceName)) {
        extendOrphans(workspaceId, clientsJson);
        break;
      }

      // If the real workspace exists on this monitor, use its actual
      // Hyprland data.
      if (workspaceJson["monitor"].asString() == m_bar.output->name) {
        m_workspacesToCreate.emplace_back(workspaceJson, clientsJson);
        break;
      }

      // The paired workspace exists on the other monitor.
      // Create a visual representation using this monitor's real ID.
      Json::Value pairedWorkspace = workspaceJson;
      pairedWorkspace["id"] = realId;
      pairedWorkspace["name"] = std::to_string(realId);
      pairedWorkspace["monitor"] = m_bar.output->name;

      m_workspacesToCreate.emplace_back(pairedWorkspace, clientsJson);
      break;
    }

    // The real workspace may not exist on this monitor. If the paired
    // workspace exists on the other monitor, create the representation
    // locally.
    bool foundLocalRepresentation = std::ranges::any_of(
        m_workspacesToCreate,
        [realId](const auto& entry) {
          return entry.first["id"].asInt() == realId;
        });

    if (foundLocalRepresentation) {
      continue;
    }
    ...
    for (const Json::Value& workspaceJson : workspacesJson) {
      ...
      const int workspaceLogicalId =
          workspaceId <= 9 ? workspaceId : workspaceId - 9;

      if (workspaceLogicalId != logicalId) {
        continue;
      }

      Json::Value pairedWorkspace;
      pairedWorkspace["id"] = realId;
      pairedWorkspace["name"] = std::to_string(realId);
      pairedWorkspace["monitor"] = m_bar.output->name;
      pairedWorkspace["windows"] = workspaceJson["windows"].asInt();
  ...
  // Keep the normal persistent-workspace functionality intact if the
...
void Workspaces::onWorkspaceDestroyed(std::string const& payload) {
  ...
  const int destroyedId = *workspaceId;

  // Only the paired workspaces 1<->10 through 9<->18 participate
  // in the cross-monitor display.
  if (destroyedId < 1 || destroyedId > 18) {
    m_workspacesToRemove.push_back(workspaceIdStr);
    return;
  }

  const int logicalId =
      destroyedId <= 9 ? destroyedId : destroyedId - 9;

  const int pairedId =
      destroyedId <= 9 ? destroyedId + 9 : destroyedId - 9;
  ...
  bool pairStillInUse = false;

  for (const Json::Value& workspaceJson : workspacesJson) {
    const int currentId = workspaceJson["id"].asInt();

    if (currentId != destroyedId && currentId != pairedId) {
      continue;
    }

    const int currentLogicalId =
        currentId <= 9 ? currentId : currentId - 9;

    if (currentLogicalId != logicalId) {
      continue;
    }

    const int windows = workspaceJson["windows"].asInt();

    if (windows > 0 || currentId == activeWorkspaceId) {
      pairStillInUse = true;
      break;
    }
  }
  ...
  if (!pairStillInUse) {
    // Neither side of the pair is being used anymore.
    m_workspacesToRemove.push_back(workspaceIdStr);
  } else {
    // The other monitor still has this logical workspace in use,
    // so keep the visual button on this monitor.
    spdlog::debug(
        "Keeping paired workspace {} because workspace pair {} is still in use",
        destroyedId,
        logicalId);
  }
}
...
void Workspaces::onWorkspaceCreated(
  ...
  const int createdId = *workspaceId;
  ...
  // Normal/special workspace handling.
  for (Json::Value workspaceJson : workspacesJson) {
    ...
    if (workspaceName.starts_with("special") &&
        !showSpecial()) {
      return;
    }
    ...
    // Apply the normal Hyprland workspace-rule persistence state.
    for (const Json::Value& rule : workspaceRules) {
    ...
    // Workspace is actually on this monitor.
    if (allOutputs() ||
        m_bar.output->name ==
  ...
  // Cross-monitor paired workspace handling.
  if (createdId < 1 || createdId > 18) {
    return;
  }

  const int logicalId =
      createdId <= 9 ? createdId : createdId - 9;
  ...
  // Don't add another copy if the real workspace was already
  // queued for this monitor.
  bool alreadyQueued = std::ranges::any_of(
      m_workspacesToCreate,
      [localId](const auto& entry) {
        return entry.first["id"].asInt() == localId;
      });
  ...
  // Find either side of the pair.
  for (const Json::Value& workspaceJson : workspacesJson) {
    const int currentId = workspaceJson["id"].asInt();

    if (currentId < 1 || currentId > 18) {
      continue;
    }
    ...
    // If this monitor owns the real workspace, it was already
    // handled above.
    if (workspaceJson["monitor"].asString() ==
        m_bar.output->name) {
      return;
    }

    // The workspace exists on the other monitor.
    // Create a visual representation here using this monitor's
    // real Hyprland workspace ID.
    Json::Value pairedWorkspace = workspaceJson;

    pairedWorkspace["id"] = localId;
    pairedWorkspace["name"] =
        std::to_string(localId);
    pairedWorkspace["monitor"] =
        m_bar.output->name;
